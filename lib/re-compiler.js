  ...
  kleene: (stack, optimize) => {
    ...
    if (optimize) {
      ...
      if (x.trailing !== 'kleene') {
        ...
        // The trailing op of x is also kleene, to ensure the correctness, we
        // have to add another lambda transition from x.final to x.init, and let
        // the original initial state also be the final state.
        edges = [
          {
            name: 'λ',
            from: x.final,
...
const close = function(stateSet, edges) {
  // Find all λ-successors of all states in the stateSet.
  let ans = stateSet;
  let changed = false;
  ...
  do {
    changed = false;
    for (s of ans) {
      // Find all lambda transitions from s.
      let transitions = edges.filter(e => e.from === s && e.name === 'λ');
...
const match = function(stringToMatch) {
  let tokenStream = stringToMatch.split('').filter(c => c !== '^' && c !== 'λ');
  let traversal = [];

  let last = close(new Set([this.init]), this.edges);
  traversal.push(last);
  ...
  while (tokenStream.length > 0) {
    ...
    for (state of last) {
      this.edges.filter(e => e.from === state && e.name === tokenStream[0])
                .forEach(e => successors.add(e.to));
    }

    last = close(successors, this.edges);
    if (!last.size) break;  // no transitions available

    traversal.push(last);
    tokenStream.shift();
  }

  // See if the last step contains a final.
  return this.final.some(s => last.has(s));
}

const getDotScript = function() {
  // Utilize the javascript-state-machine package to generate the dot script.
  let final = [];
  let transitions = [];
...
const generateNfa = function(code, optimize) {
  ...
  nfa.match = match;  // set the match method for nfa
  nfa.getDotScript = getDotScript;  // set the method to get dot script
  return nfa;
}
